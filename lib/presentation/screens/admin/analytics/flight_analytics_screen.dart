...
class _FlightAnalyticsScreenState extends State<FlightAnalyticsScreen> {
    ...
    return Scaffold(
      ...
      body: Consumer<AnalyticsProvider>(
        builder: (context, provider, child) {
          ...
          final analytics = provider.flightAnalytics;
          if (analytics == null) {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStatCard(
                    'Total Flights',
                    analytics['totalFlights']?.toString() ?? '0',
                  ...
                  _buildStatCard(
                    'On-Time Flights',
                    analytics['onTimeFlights']?.toString() ?? '0',
                    Icons.check_circle,
                    AppColors.success,
                  ),
                  SizedBox(height: 12),
                  _buildStatCard(
                    'Delayed Flights',
                    analytics['delayedFlights']?.toString() ?? '0',
                    Icons.schedule,
                    AppColors.warning,
                  ),
                  SizedBox(height: 12),
                  _buildStatCard(
                    'Cancelled Flights',
                    analytics['cancelledFlights']?.toString() ?? '0',
                    Icons.cancel,
                    AppColors.error,
                  ),
                  SizedBox(height: 24),
                  ...
                  SizedBox(height: 16),
                  ..._buildPopularRoutes(analytics['popularRoutes']),
                  SizedBox(height: 24),
                  Text(
                    'Flight Performance',
                    style: TextStyle(
                      ...
                      child: Column(
                        children: [
                          _buildPerformanceRow(
                            'On-Time Rate',
                            analytics['onTimeRate']?.toString() ?? '0%',
                            AppColors.success,
                          ),
                          Divider(height: 24),
                          _buildPerformanceRow(
                            'Average Delay',
                            analytics['averageDelay']?.toString() ?? '0 min',
                            AppColors.warning,
                          ),
                          Divider(height: 24),
                          _buildPerformanceRow(
                            'Cancellation Rate',
                            analytics['cancellationRate']?.toString() ?? '0%',
                            AppColors.error,
                          ),
                        ],
                      ),
  ...
  List<Widget> _buildPopularRoutes(dynamic routes) {
    if (routes == null) return [];

    List<Widget> widgets = [];
    for (var route in routes) {
      widgets.add(_buildRouteCard(
        route['route'] ?? '',
        route['count'] ?? 0,
        AppColors.primary,
      ));
